...
# Muat data di awal agar sidebar bisa menampilkan statistik cepat (opsional)
df_spam, data_loaded = load_spam_data()
report_content, report_loaded = load_report()

# ===============================
# SIDEBAR + NAVIGASI UTAMA (FIX)
# ===============================
st.sidebar.markdown("## 🎯 Navigation")

page_options = ["📊 Overview & Metrics", "📋 Detailed Data", "📈 Visualizations", "📝 Reports", "💡 Insights"]

# Gunakan key YANG SAMA: 'page_selection' sebagai satu-satunya sumber kebenaran
st.sidebar.selectbox(
    "Choose Analysis Section:",
    page_options,
    index=page_options.index(st.session_state.page_selection) if 'page_selection' in st.session_state else 0,
    key="page_selection"
)
...
# Quick Stats Sidebar (opsional: tampilkan dinamis jika data ada)
...
# Tombol Quick Navigation (mengubah st.session_state.page_selection lalu rerun)
...
# ===============================
# KONTEN HALAMAN BERDASARKAN SELEKSI
# ===============================
if st.session_state.page_selection == "📊 Overview & Metrics":
    ...
    if data_loaded and df_spam is not None:
        # Opsi filter (jika kolom tersedia)
        if 'spam_classification' in df_spam.columns:
